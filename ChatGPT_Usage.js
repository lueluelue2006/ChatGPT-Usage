...
(function () {
    ...
    // ==== 用户配置（可在此修改） ====
    // 将下方改为 true 开启静默模式（只采集、不显示界面）
    // 改为 false 则正常显示监视器界面
    const SILENT_MODE = false; // 感谢 LengmoAngel（linux.do）提供建议
    // ==== 用户配置结束 ====
    ...
    const defaultUsageData = {
        ...
        planType: "team", // 默认套餐：team, plus, free, edu, enterprise, pro, go, k12_teacher
    ...
    // 套餐配置
    // 套餐显示顺序
    const PLAN_DISPLAY_ORDER = ["free", "go", "k12_teacher", "plus", "team", "edu", "enterprise", "pro"]; // 其余套餐排在后面
...
`);
    ...
    const Storage = {
        ...
        get() {
            ...
            // 静默模式（只采集不显示UI）
            if (usageData.silentMode === undefined) {
                usageData.silentMode = false;
            }
            // 顶部用户配置优先级最高
            if (typeof SILENT_MODE === "boolean") {
                usageData.silentMode = SILENT_MODE;
            }
...
<body>
    ...
    function createMonitorUI() {
        // 静默模式下不创建任何 UI
        if (usageData && usageData.silentMode) {
            return;
        }
    ...
    function initialize() {
        ...
        // 静默模式：只采集数据，不显示 UI 和快捷键
        if (usageData.silentMode) {
            const existingMonitor = document.getElementById("chatUsageMonitor");
            if (existingMonitor) {
                existingMonitor.remove();
            }
            console.log("[monitor] Silent mode enabled: tracking without UI");
            return;
        }
    ...
    const observer = new MutationObserver(() => {
        // 静默模式下不尝试重新创建 UI
        if (usageData && usageData.silentMode) return;
